...
# NESY_SOLVER ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libnesy_solver.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── nesy_solver.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── nesy_solver.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/nesy_solver.h
```
...
```c
#include "nesy_solver.h"

int main() {
    void* handle = nesy_solver_init();
    if (!handle) return 1;

    int result = nesy_solver_process(handle, 42);
    if (result != 0) {
        const char* err = nesy_solver_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    nesy_solver_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lnesy_solver -L./zig-out/lib
```

### From Idris2

```idris
import NESY_SOLVER.ABI.Foreign
...
```rust
#[link(name = "nesy_solver")]
extern "C" {
    fn nesy_solver_init() -> *mut std::ffi::c_void;
    fn nesy_solver_free(handle: *mut std::ffi::c_void);
    fn nesy_solver_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = nesy_solver_init();
        assert!(!handle.is_null());

        let result = nesy_solver_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libnesy_solver = "libnesy_solver"

function init()
    handle = ccall((:nesy_solver_init, libnesy_solver), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:nesy_solver_process, libnesy_solver), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:nesy_solver_free, libnesy_solver), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/nesy_solver.h
   ```
