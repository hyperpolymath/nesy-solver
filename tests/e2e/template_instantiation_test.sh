...
find "$TEST_REPO_PATH" -type f \
    ...
    -exec bash -c '
        file="$1"
        placeholder_pairs=(
            "nesy-solver|$TEST_REPO_NAME"
            "hyperpolymath|$TEST_OWNER"
            "github.com|$TEST_FORGE"
            "NESY_SOLVER|$TEST_PROJECT_NAME"
            "nesy_solver|'"${TEST_REPO_NAME//-/_}"'"
            "Live proof playground for neurosymbolic theorem provers at nesy-solver.dev|$TEST_DESCRIPTION"
            "ReScript|$TEST_PRIMARY_LANGUAGE"
