...
verify-template:
    ...
    if grep -q 'nesy-solver' Justfile 2>/dev/null; then
        echo "⚠ Justfile still references 'nesy-solver' — update project name"
        FOUND=1
    fi
...
build *args:
    @echo "Building nesy_solver (debug)..."
...
build-release *args:
    @echo "Building nesy_solver (release)..."
...
cookbook:
    ...
    echo "= nesy_solver Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/nesy_solver.1 << EOF
    .TH nesy_solver 1 "$(date +%Y-%m-%d)" "{{version}}" "nesy_solver Manual"
    .SH NAME
    nesy_solver \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/nesy_solver.1"
...
container-init:
    ...
    if [ ! -d "container" ]; then
        ...
        echo "This repo may not have been created from nesy-solver."
        exit 1
    fi
    ...
    read -rp "Service name (e.g. my-api) [nesy_solver]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-nesy_solver}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t nesy_solver:latest -f container/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t nesy_solver:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push nesy_solver:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it nesy_solver:latest {{args}}
...
doctor:
    @echo "Running diagnostics for nesy-solver..."
...
help-me:
    @echo "=== nesy-solver Help ==="
    @echo "Platform: $(uname -s) $(uname -m)"
    @echo "Shell: $SHELL"
    @echo ""
    @echo "To report an issue:"
    @echo "  https://github.com/hyperpolymath/nesy-solver/issues/new"
    @echo ""
